...
function initialSlider(slider) {
  ...
  function theChecker() {
    ...
    slideElementNumber.textContent =
      "Image " + currentSlide + " of " + sliderCount;
    //Remove All Active Classes
    removeAllActive();
    //Set Active class on the current slide
    sliderImages[currentSlide - 1].classList.add("active");
    sliderImages[currentSlide - 1].style.cssText =
      "z-index: 2; display: block; opacity: 0;";
    setTimeout(() => {
  ...
  function fullScreenPrevImage() {
    if (popupCurrentImg > 1) {
      popupCurrentImg--;
      slider.querySelector(".popup-img img").src =
        sliderImages[popupCurrentImg - 1].src;
    }
  }
